...
function Events(o,handle){
	...
	//exp:  
	//	on('alert',function(){...});
	E['on']=E['bind']= function(methed, callback){
		var i,len;
		if(!o[methed]) return;
	...
	//	解除所有事件的所有函数 off();
	E['off']=E['unbind']= function(methed, callback){
		var m,i,len,list;
		if(!callback && !methed){
			for(m in o){
	...
	//exp:
	//	fire('alert')
	E['fire']=E['trigger']= function(methed){
		var list,event;
		o[methed] 
			&& o[methed].prototype 
			&& o[methed].list 
			&& (list = o[methed].list);
		while(event = list.shift()){
			event && event();
		}
	}
